import { Wind, Trash2, HeartPulse, MapPin, ArrowRight, CloudRain, AlertTriangle, Leaf, LogOut, User } from 'lucide-react';
...
interface PageProps {
    auth: {
        citizen: AuthCitizen | null;
    };
}

export default function Welcome() {
    // Correctly typing and extracting shared props via Inertia usePage
    ...
    return (
        <>
            ...
            <div className="min-h-screen bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-50 selection:bg-emerald-500 selection:text-white font-sans overflow-hidden relative">
                ...
                {/* Navbar */}
                <header className="relative z-20 container mx-auto px-6 py-6 flex justify-between items-center animate-fade-in-up">
                        ...
                        {auth && auth.citizen ? (
                            <div className="flex items-center gap-4 bg-white dark:bg-slate-900 px-4 py-2 rounded-full border border-slate-200 dark:border-slate-800 shadow-sm">
                                <div className="flex items-center gap-2 text-sm font-medium text-slate-700 dark:text-slate-200">
                                    ...
                                    <span>{auth.citizen.name} (Ward {auth.citizen.ward_no})</span>
                                </div>
                                ...
                                <Link
                                    ...
                                    method="post"
                                    as="button"
                                    className="flex items-center gap-1.5 text-sm font-semibold text-rose-600 hover:text-rose-500 dark:text-rose-400 dark:hover:text-rose-300 transition-colors cursor-pointer"
                                >
                ...
                <main className="relative z-10 container mx-auto px-6 pt-20 pb-24 text-center">
                    ...
                    <div className="flex flex-col sm:flex-row justify-center gap-4 animate-fade-in-up" style={{ animationDelay: '300ms' }}>
                        <Link
                            href={auth && auth.citizen ? '#features' : citizenRegisterPage()}
                            className="group px-8 py-3.5 rounded-full bg-emerald-500 text-white font-semibold hover:bg-emerald-600 transition-all shadow-lg shadow-emerald-500/25 flex items-center justify-center gap-2 hover:-translate-y-1"
                        >
                            {auth && auth.citizen ? 'Explore Metrics' : 'Get Started'}
